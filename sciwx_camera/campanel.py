...
class CameraPanel ( wx.Panel ):
    ...
    def __init__( self, parent, id = wx.ID_ANY, pos = wx.DefaultPosition, size = wx.Size( 600, 600 ), style = wx.TAB_TRAVERSAL, name = wx.EmptyString ):
        ...
        self.tol_open = self.tool_bar.AddTool( wx.ID_ANY, u"Open", wx.Bitmap('./next.png'), wx.NullBitmap, wx.ITEM_NORMAL, wx.EmptyString, wx.EmptyString, None )

        self.tol_start = self.tool_bar.AddTool( wx.ID_ANY, u"Start", wx.Bitmap('./next.png'), wx.NullBitmap, wx.ITEM_NORMAL, wx.EmptyString, wx.EmptyString, None )

        self.tol_stop = self.tool_bar.AddTool( wx.ID_ANY, u"Stop", wx.Bitmap('./next.png'), wx.NullBitmap, wx.ITEM_NORMAL, wx.EmptyString, wx.EmptyString, None )

        self.tol_close = self.tool_bar.AddTool( wx.ID_ANY, u"Close", wx.Bitmap('./next.png'), wx.NullBitmap, wx.ITEM_NORMAL, wx.EmptyString, wx.EmptyString, None )
        ...
        bSizer1.Add( self.canvas, 1, wx.EXPAND |wx.ALL, 0 )
        self.canvas.set_img(np.zeros((512, 512), np.uint8))
    ...
    def hold(self):
        while self.status:
            ...
            if img.shape != self.canvas.image.img.shape:
                wx.CallAfter(self.canvas.set_img, img)
            else:
                ips = self.canvas.image
                cache = self.spn_cache.GetValue()
                ips.imgs = (ips.imgs + [img])[-cache:]
                ips.cur = len(ips.imgs) - 1
                wx.CallAfter(self.canvas.update)
